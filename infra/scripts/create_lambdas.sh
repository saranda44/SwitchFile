...
for FUNC_NAME in $LAMBDA_NAMES; do
    ...
    case "$FUNC_NAME" in
        ...
        enqueueSQS)
            ...
            ;;
        getFiles)
            ENV_VARS="Variables={DYNAMODB_TABLE_FILES=$TABLE_FILES}"
            ;;
        getFileDetail)
            ENV_VARS="Variables={DYNAMODB_TABLE_FILES=$TABLE_FILES}"
            ;;
        getVault)
            ENV_VARS="Variables={DYNAMODB_TABLE_CONVERSIONS=$TABLE_CONVERSIONS,S3_BUCKET_CONVERTED=$BUCKET_CONVERTED}"
            ;;
        getVaultDetail)
            ENV_VARS="Variables={DYNAMODB_TABLE_FILES=$TABLE_FILES,DYNAMODB_TABLE_CONVERSIONS=$TABLE_CONVERSIONS}"
            ;;
        download)
            ENV_VARS="Variables={S3_BUCKET_CONVERTED=$BUCKET_CONVERTED}"
            ;;
        upload)
            ENV_VARS="Variables={S3_BUCKET_UPLOADS=$BUCKET_UPLOADS,STEP_FUNCTION_ARN=$STEP_FUNCTION_ARN}"
            ;;
        reconvert)
            ENV_VARS="Variables={DYNAMODB_TABLE_FILES=$TABLE_FILES,SQS_QUEUE_URL=$SQS_QUEUE_URL}"
            ;;
        *)
            ENV_VARS=""
            ;;
